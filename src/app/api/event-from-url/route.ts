...
/**
 * X/Twitter URL は fxtwitter.com に書き換える
 * fxtwitter は OGP (og:title, og:description, og:image) をツイート内容で正しく埋めて返すプロキシ
 */
function rewriteXUrl(url: string): string {
  try {
    ...
    if (u.hostname === 'x.com' || u.hostname === 'www.x.com' ||
      ...
      // fxtwitter.com は OGP (og:title/og:description/og:image) を
      // ツイート内容で埋めたHTMLを返すプロキシ（Discord/Slack embed互換）
      u.hostname = 'fxtwitter.com'
      u.search = '' // トラッキングパラメータを除去
      return u.toString()
    }
  } catch { /* noop */ }
  return url
...
export async function POST(request: Request) {
  try {
    ...
    // ─── Step 1: HTML取得（X/Twitter は fxtwitter.com 経由で OGP 取得） ──
    const fetchUrl = rewriteXUrl(url)
    const htmlRes = await fetch(fetchUrl, {
      headers: {
        // bot UA を明示: X/TwitterやOGP系は bot UA で綺麗なOGPを返す
