...
- **eplus.jp (e+)**: チケット種別ごとに価格が並ぶ（S席/A席/指定席/自由席）。自由席価格を price に、他を memo に。
...
export async function POST(request: Request) {
  try {
    ...
    if (xData) {
      ...
      // 直接fetchが失敗 or OGPが汎用テンプレの場合は Jina Reader にフォールバック
      const needsJina = !directOk || isGenericOgp(pageTitle, pageDescription, textContent)
      if (needsJina) {
